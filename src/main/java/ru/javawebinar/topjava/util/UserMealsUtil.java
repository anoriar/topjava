...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExceed> getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        for (UserMeal meal : mealList) {
            ...
            if (!caloriesAmount.containsKey(date)) {
                caloriesAmount.put(date, meal.getCalories());
            } else {
                caloriesAmount.put(date, caloriesAmount.get(date) + meal.getCalories());
            }
        }
        ...
        for (UserMeal meal : mealList) {
            ...
            if (time.isAfter(startTime) && time.isBefore(endTime)) {
                ...
                if (caloriesAmount.getOrDefault(date, 0) > caloriesPerDay) {
                    exceed = true;
                }
    ...
    */
    ...
    public static List<UserMealWithExceed> getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        Map<LocalDate, Integer> calSumByDate = mealList
                .stream()
        ...
        return mealList.stream()
                ...
                .map(meal -> new UserMealWithExceed(meal.getDateTime(), meal.getDescription(), meal.getCalories(),
                        calSumByDate.get(meal.getDateTime().toLocalDate()) > caloriesPerDay))
